		...
		try {
			...
			*****************************************/
			...
			while (rs.next()) {
				...
				+ rs.getInt("idx") + "&pgnum="+pageSeq+"'>" +
				// 조회수정 페이지인 modify.jsp로 갈때
				// ?idx=유일키값 : Get방식으로 전송함!
				// pgnum=현재페이지번호 : 추가전송!
				rs.getString("dname") + "</a></td>" + "<td>" 
				+ rs.getString("actors") + "</td>" + "<td>"
				+ rs.getString("broad") + "</td>" + "<td>" 
				+ rs.getString("gubun") + "</td>" + "<td>"
				+ rs.getString("stime") + "</td>" + "<td>" 
				+ rs.getString("total") + "</td>" + "</tr>";
